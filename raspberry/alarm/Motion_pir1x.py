...
handler = RotatingFileHandler("/var/www/html/motion.log", maxBytes=200000,backupCount=5)
		...
		while(1):
			...
			try:
				...
				if is_false_positive() is False:		
					...
					if GPIO.input(12) == 0:				
						myfile = open("/var/www/html/motion.log", "ab")
						fcntl.flock(myfile, fcntl.LOCK_EX)				
						myfile.write(time.strftime("%d/%m/%Y-%H:%M:%S   PIR1: siren on \n"))
						GPIO.output(25,True) # Red Led
						GPIO.output(12,True) # trigger transistor base to siren
